...
if (-not(get-AzResourceGroup -Name $rgname -ErrorAction silentlyContinue )) {
    ...
    if ($response.Trim().ToUpper() -eq "Y") {
        ...
        Write-Output "Creating Public IP Address: $publicIpName"
        New-AzPublicIpAddress -Name $publicIpName -ResourceGroupName $rgname `
            -Location $location -AllocationMethod Dynamic -DnsName $dnsPrefix
    }
    else {
